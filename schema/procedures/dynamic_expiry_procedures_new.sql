DROP PROCEDURE IF EXISTS `expire_impression_counter_events_new`;
DELIMITER $$
CREATE PROCEDURE expire_impression_counter_events_new (_interval TINYINT,
									 _chunk_size INT)
BEGIN
	SET @x2 = 0;
	DECLARE CONTINUE HANDLER FOR SQLSTATE '40001' SET @x2 = @x2 + 1;
	...
	SET @EVENT_NAME=CONCAT('expire_impression_click_new_day_minus_', _interval);
	...
	IF(@EXECUTION_DAY < @today) THEN
		...
		SET @min_id = 0;
		SET @max_id = 0;
		SET @MIN_MAX_ID_QUERY = CONCAT('SELECT IF(min(id) IS NULL,-1,min(id)), IF(max(id) IS NULL,-1,max(id)) INTO @min_id, @max_id FROM device_impressions_daily WHERE day_int=',to_days(@EXECUTION_DAY), ' and status = \'active\'');
		CALL execute_query(@MIN_MAX_ID_QUERY);
-- loop over threads
		WHILE @thread_loop < 1 DO
	        	SET @rowcount = 0;
	        	SET @rowcount_temp = 0;
			SET @start_id = 0;
			...
			SET @UPDATE_STATEMENT = CONCAT(@UPDATE_STATEMENT, ' AND id>=REPLACE_START_ID AND id<=REPLACE_END_ID AND status=\'active\' and thread_id = ',@thread_loop);
